...
// the function that is called after completion of writing.
///Release resources
void echo_write(uv_write_t* req, int status) {
    if (status) {
...
//         printf("There are a new connection\n");
//         //pthread_exit(NULL);
...
void *thread_handle_http_client_connect(uv_stream_t* client)
{
...
void on_new_connection(uv_stream_t* server, int status) {
    ...
    if (uv_accept(server, (uv_stream_t*)&client->handle) == 0) {
        ...
        client->parser.data = client;
        //uv_read_start((uv_stream_t*)client, alloc_buffer, echo_read);
        uv_thread_create(&hare_id, (void *) thread_handle_http_client_connect,(uv_stream_t*) client);
    } else {
        uv_close((uv_handle_t*)client, NULL);
    }
    printf("done on_new_connection\n");
